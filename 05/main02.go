...
const gridSize = 1000 // 10 for example
...
func addLineToDiagram(d [gridSize][gridSize]int, l line) [gridSize][gridSize]int {
	if l.from.y == l.to.y && l.from.x != l.to.x { // run on the x
		steps, direction := getStepsAndDirection(l.from.x, l.to.x)
		for i := 0; i <= steps; i++ {
			d[l.from.x+(i*direction)][l.from.y]++
		}
	} else if l.from.x == l.to.x && l.from.y != l.to.y { // run on the y
		steps, direction := getStepsAndDirection(l.from.y, l.to.y)
		for i := 0; i <= steps; i++ {
			d[l.from.x][l.from.y+(i*direction)]++
		}
	} else {
		steps, dirX, dirY := getStepsAndDirectionDiagonal(l)
		for x := 0; x <= steps; x++ {
			newX := l.from.x + (x * dirX)
			newY := l.from.y + (x * dirY)
			d[newX][newY]++
		}
	}
	return d
...
func getStepsAndDirectionDiagonal(l line) (int, int, int) {
	stepsX := l.to.x - l.from.x
	dirX := 1
	if stepsX < 0 {
		dirX = -1
		stepsX = int(math.Abs(float64(stepsX)))
	}
	dirY := 1
	if l.to.y-l.from.y < 0 {
		dirY = -1
	}
	return stepsX, dirX, dirY // diagonal it's same steps alwayss
}
